...
if (!Number.isFinite(ts) || Date.now() - ts > LIVE_STALE_MS) {
  return res.json({});
}
...
router.get("/tracking/history", async (req, res) => {
  try {
    ...
    const newest = result.recordset[0];
      if (!newest) return res.json([]);

    const newestTs = Number(newest.Timestamp);
      if (!Number.isFinite(newestTs) || Date.now() - newestTs > LIVE_STALE_MS) {
        return res.json([]);
    }
